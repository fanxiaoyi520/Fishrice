...
@interface ZDACodeLoginViewController ()
...
@implementation ZDACodeLoginViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.naviTitle = @"短信驗證碼登錄";
...
- (void)setUI
{
    ...
    [titleArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger i, BOOL * _Nonnull stop) {
        ...
        CGRect actionrect = [ZDPayFuncTool getStringWidthAndHeightWithStr:actionArray[0] withFont:label_font_PingFangSC_Regular(16)];
        UIButton *actionBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        if (i==0)actionBtn.frame = CGRectMake(20, logoImageView.bottom+204, actionrect.size.width, 16);
        if (i==0)[self.view addSubview:actionBtn];
        ...
        [actionBtn setTitle:actionArray[0] forState:UIControlStateNormal];
...
- (void)networking_Login
{
    ZDLoginModel *model = [ZDLoginModel mj_objectWithKeyValues:self.mutableDic];
    NSString *str = [self.iphonePrefixStr stringByReplacingOccurrencesOfString:@"+" withString:@""];
    //NSString *language = [ZDPayFuncTool getPreferredLanguage];
    NSString *userName = [NSString stringWithFormat:@"%@",model.userName];
    NSDictionary *params = @{
        @"account": userName,
        @"areaCode": str,
        @"source": @"app",
        @"varifyCode": [self.mutableDic valueForKey:@"password"],
    };
    [MBProgressHUD showHUDAddedTo:self.view animated:YES];
    [AFNetworkingManager requestWithType:SkyHttpRequestTypePost urlString:[NSString stringWithFormat:@"%@%@",DOMAINNAME,LOGIN_SHORTMESSAGE] parameters:params successBlock:^(id  _Nonnull responseObject) {
        [MBProgressHUD hideHUDForView:self.view animated:YES];
        NSString *code = [NSString stringWithFormat:@"%@",responseObject[@"code"]];
        if ([code isEqualToString:@"200"]) {
